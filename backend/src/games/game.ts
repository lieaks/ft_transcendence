...
export class Game implements IGame {
  ...
  async finish(): Promise<void> {
    ...
    const loser =
      this.score.left > this.score.right ? this.players[1] : this.players[0];
  ...
  updateScore(): void {
    if (
      ...
      if (
        this.score.left === this.scoreLimit ||
        this.score.right === this.scoreLimit
      ) {
        this.finish();
      }
    }
  ...
  reset(): void {
    ...
    this.emitToPlayers('updatePaddlePosition', {
      player: 'left',
      y: this.leftPaddle.y,
    });
    this.emitToPlayers('updatePaddlePosition', {
    ...
    if (player.id === this.players[0].id) {
      if (direction === 'up' && this.leftPaddle.y > this.leftPaddle.speed - 2) {
        ...
        this.leftPaddle.y + this.leftPaddle.height <
          this.canvas.height - this.leftPaddle.speed + 2
      ) {
        this.leftPaddle.y += this.leftPaddle.speed;
      }
      this.emitToPlayers('updatePaddlePosition', {
      ...
      if (
        direction === 'up' &&
        this.rightPaddle.y > this.rightPaddle.speed - 2
      ) {
        this.rightPaddle.y -= this.rightPaddle.speed;
      } else if (
        direction === 'down' &&
        this.rightPaddle.y + this.rightPaddle.height <
          this.canvas.height - this.rightPaddle.speed + 2
      ) {
        this.rightPaddle.y += this.rightPaddle.speed;
      }
      this.emitToPlayers('updatePaddlePosition', {
  ...
  update(): void {
    ...
    if (
      ...
      this.ball.dx = -this.ball.dx;
      this.ball.x =
        this.rightPaddle.x - this.rightPaddle.width - this.ball.radius;
    }
